---
name: capo
description: Drive the CAPO CLI to run one AI agent team across Claude Code and Codex. Use when the user wants to start, check, switch, or resume a CAPO run, or when the user says they are close to a Codex usage limit and wants to keep the run going on Claude Code. Also use for "capo run", "capo status", "capo switch", "capo resume", "capo doctor", or any request to move an agent team to the other platform before it hits a limit.
---
...
does not reimplement anything the CLI does. There is no MCP server here: run
the CLI directly.
...
If `${CLAUDE_PLUGIN_ROOT}` is not substituted in your environment, fall back to
the path relative to this skill file: `../../dist/capo.mjs`. Confirm with
`node <path> --version`, which prints the CAPO version and nothing else.

The bundled CLI lives at `dist/capo.mjs` inside this plugin's own installed
directory. Resolve it with the plugin-root variable this host provides so it
never depends on a development checkout or a global npm install:
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/dist/capo.mjs" switch 2026-09-12-001 --to claude
```
...
Both plugins — this one and the Claude Code one — read and write the same
`.capo/` directory in the project. Starting a run from Codex and checking on
it from Claude Code, or the reverse, is the normal flow: after a switch, this
very conversation may itself be on the platform that is now capped.
